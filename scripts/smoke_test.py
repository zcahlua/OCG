...
pos = torch.tensor([[0.,0,0],[1,0,0],[0,1,0],[0,0,1.], [4.,0,0],[5.,0,0],[4.,1,0]], dtype=torch.float32)
...
with torch.no_grad():
    yt = model(z=z, pos=pos + torch.tensor([2., -1., .5]), batch=batch)
    A = torch.tensor([[0.,1.,0.],[1.,0.,0.],[0.,0.,-1.]])
    yr = model(z=z, pos=pos @ A.T + torch.tensor([1.,2.,3.]), batch=batch)
    p = torch.tensor([2,0,3,1, 6,4,5]); inv_batch = batch[p]
...
print('max rotation/reflection-invariance difference:', float((y.detach()-yr).abs().max()))
