...
def sets(x):
    return {tuple(r) for r in x.tolist()}


def mapped(k, inv):
    return {tuple(sorted(int(inv[i]) for i in row)) for row in k.tolist()}
...
def test_lift_permutation_equivariance():
    ...
    assert (
        sets(B.k1) == mapped(BP.k1, p)
        and sets(B.k2) == mapped(BP.k2, p)
        and sets(B.k3) == mapped(BP.k3, p)
    )
