...
def build_incidence(
    k1: Tensor, k2: Tensor, k3: Tensor, num_nodes: int
) -> dict[str, Tensor]:
    """Build boundary indices, coface maps, and constant incidence features."""
    device = k1.device
    edge_map: Dict[Tuple[int, int], int] = {
        tuple(map(int, e.tolist())): idx for idx, e in enumerate(k1)
    }
    tri_map: Dict[Tuple[int, int, int], int] = {
        tuple(map(int, t.tolist())): idx for idx, t in enumerate(k2)
    }
    ...
    tri_to_edges = (
        torch.tensor(tri_edges, dtype=torch.long, device=device)
        if tri_edges
    ...
    tet_to_tris = (
        torch.tensor(tet_tris, dtype=torch.long, device=device)
        if tet_tris
        else torch.empty((0, 4), dtype=torch.long, device=device)
    )

    node_tgt, node_src = [], []
    for e_idx, (i, j) in enumerate(k1.tolist()):
        node_tgt.extend([int(i), int(j)])
        node_src.extend([e_idx, e_idx])
    edge_tgt, edge_src = [], []
    for f_idx, edges in enumerate(tri_to_edges.tolist()):
        for e_idx in edges:
            edge_tgt.append(int(e_idx))
            edge_src.append(f_idx)
    tri_tgt, tri_src = [], []
    for t_idx, tris in enumerate(tet_to_tris.tolist()):
        for f_idx in tris:
            tri_tgt.append(int(f_idx))
            tri_src.append(t_idx)
