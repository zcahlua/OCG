...
class BoundaryOrbitEncoder(nn.Module):
    ...
    def __init__(
        ...
        if pi_mode == "id":
            perms = [tuple(range(arity))]
        elif pi_mode == "full":
            perms = list(itertools.permutations(range(arity)))
        else:
            raise ValueError("pi_mode must be 'id' or 'full'")
        self.perms = perms
        self.mlp = MLP(
            arity * (hidden_dim + inc_dim) + hidden_dim, mlp_hidden_dim, out_dim
        )

    def forward(
        self, boundary_h: Tensor, boundary_inc: Tensor, self_h: Tensor
    ) -> Tensor:
