...
class CofaceAggregator(nn.Module):
    ...
    def __init__(
        ...
        mlp_hidden_dim: int,
        aggr: str = "sum",
        learned_empty: bool = True,
    ) -> None:
        super().__init__()
        if aggr not in {"sum", "mean"}:
            raise ValueError("aggr must be 'sum' or 'mean' in v1")
        self.aggr = aggr
        self.learned_empty = learned_empty
    ...
    def forward(
        ...
        if self.aggr == "sum":
            out = scatter_sum(msg, target_index, num_targets)
        elif self.aggr == "mean":
            out = scatter_mean(msg, target_index, num_targets)
        else:
            raise ValueError("aggr must be 'sum' or 'mean' in v1")

        counts = coface_h.new_zeros((num_targets,))
        counts.index_add_(
            0,
            target_index,
            torch.ones_like(target_index, dtype=coface_h.dtype),
        )
        if self.empty is not None:
            out = torch.where(
                (counts == 0).view(-1, 1),
                self.empty.view(1, -1).to(out),
                out,
            )
        return out
