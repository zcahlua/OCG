...
class OCGSN(nn.Module):
    ...
    def __init__(
        ...
        if orbit_pool not in {"mean", "sum", "max"}:
            raise ValueError("orbit_pool must be 'mean', 'sum', or 'max' in v1")
        if pi_mode not in {"id", "full"}:
            raise ValueError("pi_mode must be 'id' or 'full' in v1")
    ...
    def expected_gamma_dims(self) -> tuple[int, int, int, int]:
        """Expected gamma0/gamma1/gamma2/gamma3 raw feature dimensions."""
        return (
            2,
            self.rbf_dim + 2,
            3 * self.rbf_dim + 10,
    ...
    def _raise_feature_mismatch(
        ...
        raise ValueError(
            ...
            f"{name} has shape {actual}, but model rbf_dim={self.rbf_dim} "
            f"expects {expected}. Rebuild the batch with the same rbf_dim "
            "as the model."
        )

    def _validate_batch_feature_dims(self, sb: SimplicialBatch) -> None:
        """Validate precomputed raw feature and permutation-node tensor shapes."""
        expected_gamma0, expected_gamma1, expected_gamma2, expected_gamma3 = (
            self.expected_gamma_dims
        )
        expected_shapes = {
            "gamma1_perm": (sb.num_edges, 2, expected_gamma1),
            "gamma2_perm": (sb.num_triangles, 6, expected_gamma2),
            "gamma3_perm": (sb.num_tetrahedra, 24, expected_gamma3),
            "edge_perm_nodes": (sb.num_edges, 2, 2),
            "tri_perm_nodes": (sb.num_triangles, 6, 3),
            "tet_perm_nodes": (sb.num_tetrahedra, 24, 4),
        }

        if sb.gamma0.dim() < 2 or int(sb.gamma0.shape[-1]) != expected_gamma0:
            self._raise_feature_mismatch(
                "gamma0",
                tuple(sb.gamma0.shape),
                (sb.num_nodes, expected_gamma0),
            )
        ...
        for name, expected in expected_shapes.items():
            ...
            if actual != expected:
                self._raise_feature_mismatch(name, actual, expected)

    def initialize_states(
        self,
        batch: SimplicialBatch,
    ) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        """Create h0/h1/h2/h3 using orbit-pooled ordered atom embeddings."""
